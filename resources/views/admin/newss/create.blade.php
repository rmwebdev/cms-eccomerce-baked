    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.newss.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('short_body') ? 'is-invalid' : '' }}" name="short_body" id="short_body" required>{{ old('short_body') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('publish_date') ? 'is-invalid' : '' }}" type="text" name="publish_date" id="publish_date" value="{{ old('publish_date') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags_id" id="tags_id" required>
                    @foreach($tags as $id => $entry)
                        <option value="{{ $id }}" {{ old('tags_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
