...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-categories.update", [$productCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="thumb">{{ trans('cruds.productCategory.fields.thumb') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.thumb_helper') }}</span>
            </div>
...
<script>
    ...
    init: function () {
@if(isset($productCategory) && $productCategory->thumb)
      var file = {!! json_encode($productCategory->thumb) !!}
