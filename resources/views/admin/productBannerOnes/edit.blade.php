...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productBannerOne.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-banner-ones.update", [$productBannerOne->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="tittle_banner">{{ trans('cruds.productBannerOne.fields.tittle_banner') }}</label>
                <input class="form-control {{ $errors->has('tittle_banner') ? 'is-invalid' : '' }}" type="text" name="tittle_banner" id="tittle_banner" value="{{ old('tittle_banner', $productBannerOne->tittle_banner) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products_id" id="products_id" required>
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('products_id') ? old('products_id') : $productBannerOne->products->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $productBannerOne->discount) }}" step="1">
